  ...
  return (
    <div>
        ...
        <table className='table user-table mb-0'>
          ...
          <tbody>
            ...
            {users.map((user, index) => (
              <User user={user} counter={index + 1 + Number(offset)} key={user.id} />
            ))}
          </tbody>
        </table>
      </div>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <button onClick={next_page} disabled={meta_data.sum_rows <= offset || meta_data.sum_rows <= limit || meta_data.sum_rows <= Number(limit) + Number(offset)}>
          {'>'}
        </button>
      </div>
    </div>
